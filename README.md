...
Auto Cat 是一个在本机运行的图片数据集整理工具，用来把杂乱的图片文件夹处理成适合 LoRA 训练的数据集。
...
- 递归读取 JPG、JPEG、PNG、WebP、BMP 和 TIFF 图片。
- 使用 SHA-256 删除内容完全相同的重复图片。
- 按总像素过滤分辨率不足的图片。
- 使用 DINOv3 或 PixAI visual embedding 分析图片相似度。
- 通过聚类和图筛选保留视觉风格一致的主要图片集合。
- 使用 Locate Anything 检测水印、签名、漫画分镜和拼图。
- 在网页中浏览筛选结果，手动移除或恢复图片。
- 使用 PixAI Tagger 生成 general tags。
- 按人数、取景和室内外比例选择训练图片。
- 清洗标签、添加 LoRA Prefix，并为每张图片生成 UTF-8 Caption。
- 输出 `manifest.json`、`pixai_tags.json` 和 `selection_report.json`，方便检查处理结果。
...
软件递归扫描所选目录，只读取支持的图片格式。每个文件会计算 SHA-256；内容完全相同的图片只保留一份。
...
软件使用 Complete-linkage 聚类。只有簇内任意两张图片都达到相似度要求时，它们才会留在同一个簇。每个簇会选出一个最能代表该簇的 medoid，并保留其他图片作为检测失败时的备用候选。
